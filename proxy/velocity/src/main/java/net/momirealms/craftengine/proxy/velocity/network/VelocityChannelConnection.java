...
public final class VelocityChannelConnection implements ProtocolStateHolder {
    private final Channel channel;
    private volatile @Nullable VelocityPlayer player; // 登录完成后绑定的玩家
    ...
    public VelocityChannelConnection(Channel channel) {
    ...
    // 返回已绑定的玩家, Velocity 登录流程未完成时为 null
    @Nullable
    ...
    public ProtocolStateHolder connection() {
        VelocityPlayer current = this.player;
        return current != null ? current : this;
    }

    // 绑定 Velocity 玩家, 并复制登录完成前捕获到的协议状态
    public void bind(VelocityPlayer player) {
