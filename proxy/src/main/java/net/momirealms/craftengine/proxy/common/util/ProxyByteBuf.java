...
public class ProxyByteBuf extends ByteBuf {
    ...
    public ProxyByteBuf(ByteBuf parent) {
        this.source = parent;
    }
    ...
    public int[] readIntIdList() {
        int listSize = this.readVarInt();
        int[] idList = new int[listSize];
        for (int i = 0; i < listSize; ++i) {
            idList[i] = this.readVarInt();
        }
        return idList;
    }

    public void writeIntIdList(int[] idList) {
        this.writeVarInt(idList.length);
        for (int id : idList) {
            this.writeVarInt(id);
        }
    }
    ...
    public <K, V, M extends Map<K, V>> M readMap(IntFunction<M> mapFactory, ProxyByteBuf.Reader<K> keyReader, ProxyByteBuf.Reader<V> valueReader) {
    ...
    public <K, V> Map<K, V> readMap(ProxyByteBuf.Reader<K> keyReader, ProxyByteBuf.Reader<V> valueReader) {
        return this.readMap(HashMap::new, keyReader, valueReader);
    }

    public <K, V> void writeMap(Map<K, V> map, ProxyByteBuf.Writer<K> keyWriter, ProxyByteBuf.Writer<V> valueWriter) {
    ...
    public void readWithCount(Consumer<ProxyByteBuf> consumer) {
    ...
    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    public <T> void writeOptional(Optional<T> value, ProxyByteBuf.Writer<T> writer) {
        if (value.isPresent()) {
    ...
    public <T> Optional<T> readOptional(ProxyByteBuf.Reader<T> reader) {
        return this.readBoolean() ? Optional.of(reader.apply(this)) : Optional.empty();
    }

    @Nullable
    public <T> T readNullable(ProxyByteBuf.Reader<T> reader) {
        return this.readBoolean() ? reader.apply(this) : null;
    }

    public <T> void writeNullable(@Nullable T value, ProxyByteBuf.Writer<T> writer) {
        if (value != null) {
            this.writeBoolean(true);
    ...
    public ProxyByteBuf writeByteArray(byte[] array) {
        this.writeVarInt(array.length);
    ...
    public ProxyByteBuf writeVarIntArray(int[] array) {
        this.writeVarInt(array.length);
        for (int value : array) {
    ...
    public ProxyByteBuf writeLongArray(long[] array) {
        this.writeVarInt(array.length);
    ...
    public ProxyByteBuf writeFixedSizeLongArray(long[] array) {
        for (long value : array) {
            this.writeLong(value);
        }
        return this;
    ...
    public ProxyByteBuf writeUUID(UUID uuid) {
    ...
    public ProxyByteBuf writeVarInt(int value) {
        while ((value & -128) != 0) {
    ...
    public ProxyByteBuf writeVarLong(long value) {
        while ((value & -128L) != 0L) {
    ...
    public ProxyByteBuf writeNbt(@Nullable Tag tag, boolean named) {
        if (tag == null) {
            this.writeByte(0);
        } else {
            try {
    ...
    public ProxyByteBuf writeUtf(String string) {
        return this.writeUtf(string, 32767);
    }

    public ProxyByteBuf writeUtf(String string, int maxLength) {
    ...
    public ProxyByteBuf writeKey(Key id) {
    ...
    @FunctionalInterface
    public interface Writer<T> extends BiConsumer<ProxyByteBuf, T> {
    ...
    @FunctionalInterface
    public interface Reader<T> extends Function<ProxyByteBuf, T> {
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof ProxyByteBuf friendlyByteBuf)) return false;
